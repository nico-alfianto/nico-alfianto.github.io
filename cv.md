# Curriculum Vitae

---

# Nico Alfianto

<p style="font-size:18px; line-height:1.6;">
🎓 Information Systems Student<br>
📊 Data Analysis & System Design Enthusiast<br>
🗄️ Passionate about Database & Business Intelligence
</p>

I am an Information Systems student with interests in Data Analytics, Business Intelligence, Database Management, and System Analysis. I enjoy transforming raw data into actionable insights through analytical thinking, data visualization, and business-oriented problem solving.

---

## Contact

📧 Email: [alfiantonico@gmail.com](mailto:alfiantonico@gmail.com)

💼 LinkedIn: https://www.linkedin.com/in/nico-alfianto

💻 GitHub: https://github.com/nico-alfianto

---

## Education

### Universitas Bina Sarana Informatika (UBSI) Bogor

**Bachelor of Information Systems**
2024 – Present

### SMAN 1 Cibinong

**Science Program**
2018 – 2021

---

## Technical Skills

### Programming & Query Languages

* Python
* SQL
* PHP
* HTML/CSS

### Data Analytics

* Data Cleaning
* Exploratory Data Analysis (EDA)
* Customer Analytics
* Business Analytics
* Predictive Analytics
* CRISP-DM

### Database & System Design

* MySQL
* Database Administration
* ERD
* UML
* SDLC

### Tools

* Power BI
* Microsoft Excel
* Git & GitHub
* Google Colab

---

## Certifications

* BNSP Certified Database Administrator – LSP BSI (2026)
* Database Systems Certification – Iner Corp (2025)
* Data Science Basics – Dicoding (2024)
* Python Basics – Dicoding (2024)
* SQL Basics – Dicoding (2024)
* Python Basics – Bisa AI (2024)
* Data Analyst – RevoU (2023)

---

## Featured Projects

### Customer Value Analysis – RFM & Logistic Regression

Customer segmentation and predictive analytics project combining RFM Analysis and Logistic Regression to identify high-value customers.

### Customer Churn Analysis

Analysis of customer retention behavior using the Telco Customer Churn dataset to identify churn drivers and business recommendations.

### Sales Performance Analysis

Business Intelligence project analyzing sales performance, profitability, and regional performance using Superstore data.

For project details, visit:

➡️ [Portfolio](/portfolio)

---

## Languages

* Indonesian (Native)
* English (Professional Working Proficiency)

---

## Download Full CV

📄 [Download PDF CV](/pdf/Nico_Alfianto_CV_Portfolio.pdf)

The PDF version contains:

* Complete Curriculum Vitae
* Professional Certifications
* Database Certifications
* Data Analytics Certifications
* Supporting Documents
